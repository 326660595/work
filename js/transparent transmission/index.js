...
{
    // 长度小于 32 的数据, 视为无效值, 不解析
    // 临时处理 controlResponse 返回的数值不正确的问题
    if(bytes.length < 16){
        return {
            length: bytes.length
        };
    ...
    */
    ...
    while (size >= offset && count <= 11) {
      if (true)
      {
        ...
        if(count == 0 && payloadObj.value == 3){
            params[payloadObj.attribute] = 1;
        }
        ...
        if(count==7){
          var payloadObj = type_dynamic(dv,count,offset);
          params[payloadObj.attribute] = parseInt(payloadObj.value); // 运动时间, 设备上报单位为 秒
          offset++;
        }
        if(count==8){
          var payloadObj = type_dynamic(dv,count,offset);
